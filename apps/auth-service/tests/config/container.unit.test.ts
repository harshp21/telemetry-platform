...
describe("AppContainer (auth-service)", () => {
  it("createContainer() returns all required properties", () => {
    const container = createContainer("auth-service", env);

    expect(container).toBeDefined();
    expect(container.serviceName).toBe("auth-service");
  ...
  it("createContainer() uses provided logger when given", () => {
    ...
    const container = createContainer("auth-service", env, mockLogger as any);
